...
test("/loop REFUSES a contract flag instead of silently dropping it", async () => {
  ...
  assert.match(out, /severity, preAuth/, "and it names exactly what you typed");
  // The refusal also catches the long spelling of the same flag.
  const long = await h.runCommand("loop", '"audit" requirePreAuth=1');
  assert.match(long, /REJECTED/);
  assert.match(long, /requirePreAuth/);
  ...
  assert.match(out, /\/goal "<objective>" severity=<value> preAuth=<value>/);
...
test("/loop explains the category= to focus= difference when that is what you typed", async () => {
  const cwd = tmpProject();
  const h = harness(cwd);
  await h.run(`new "${ROOT}" category=auth-bypass`);
